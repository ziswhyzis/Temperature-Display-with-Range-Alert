...
The objective was to design a combinational logic circuit that processes a 4-bit binary input representing a temperature reading (from 0°C to 15°C) and provides two specific outputs:
1.  **Hexadecimal Display:** The current temperature is displayed in Hex format (`0`-`F`) on a single 7-segment display.
2.  **Range Alert:** A separate "Warning" LED activates 🚨 if the temperature falls **outside** the safe operating range of 5°C to 12°C (inclusive).

**Allowed Components:** `AND`, `OR`, `NOT`, `NAND`, `NOR`, `XOR`, `X-NOR` basic gates only.

### 💡 Sample Input-Output
* **Input:** `1111` (15°C) ➡️ **Output:** Warning = `1` (ON), Display = `F`
* **Input:** `0111` (7°C)  ➡️ **Output:** Warning = `0` (OFF), Display = `7`
...
3.  **🚀 Architectural Optimization (The Common Gate Bus):**
    ...
    * We designed a **"Common Gate Bus"** consisting of shared `AND` gate configurations. 
